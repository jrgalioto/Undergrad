<form action='<? echo $_SERVER['PHP_SELF']; ?>' method='POST' name='order_form' id='order_form' enctype='multipart/form-data'>
	...
	$select_producst = "SELECT hats_id, category, description, size, keyword, code, price, stock_quantity, photo
		FROM hats
	...
	if(!$exec_select_producst){
		...
		while($one_record = mysqli_fetch_assoc($exec_select_producst)){
			...
			echo "<tr>
				...
				<td><input type='number' name='quantity[$hats_id][$price]' id='quantity' min='0' max='$max'";
					if(isset($quantity)&&!empty($quantity[$hats_id][$price])) echo "value='{$quantity[$hats_id][$price]}'";
				echo "></td></tr>";
		}
		echo "</table>";
	...
	<fieldset><legend>Payment</legend>
		<?php create_checkbox_radio_drop_down('Credit Type: ', 'radio', 'credit_type', ['required'=>'required', 'pattern'=>"^[a-zA-Z]{2,20}$", 'visa'=>'Visa', 'master'=>'Master', 'discover'=>'Discover'], $errors_array); ?>
	...
	<?php
		$select_address = "SELECT address_1, address_2, city, hat_states_id, zip from hat_customers WHERE hat_customers_id=$hat_customers_id";
		...
		if(!$exec_select_address){
			...
			$hat_states_id = $one_record['hat_states_id'];
			$zip = $one_record['zip'];
		}
		...
		create_form_field('Address 2:', 'text', 'address_2', 'address_2', ['required'=>'required', 'pattern'=>"^([1-9][0-9]*[ ,]?[a-zA-Z0-9_.# ]+)?$", 'maxlength'=>'100', 'size'=>'50', 'title'=>'Home Address', 'placeholder'=>'Suite #9'], $errors_array);
		...
		if(!$exec_select_states){
			...
			create_drop_down_from_query('State: ', 'hat_states_id', 'hat_states_id', $multi_array, ['required'=>'required', 'pattern'=>"^[1-5][0-9]?$", 'title'=>'State'], $errors_array);
		}
	...
	<fieldset><legend>Shipping Method</legend>	
		<?php
		$select_carriers_methods = "SELECT hat_carriers_methods_id, carrier, method, fee from hat_carriers_methods";
		...
		if(!$exec_select_carriers_methods){
			...
			create_drop_down_from_query('Shipping Method: ', 'hat_carriers_methods_id', 'hat_carriers_methods_id', $multi_array, ['required'=>'required', 'pattern'=>"^[0-9]{1,3}$", 'title'=>'Shipping Method'], $errors_array);
		}
		?>
	</fieldset>
	<fieldset>
