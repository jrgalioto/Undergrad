...
if(isset($_SESSION['hat_customers_id']) && isset($_SESSION['full_name'])){
	...
	if(isset($_POST['submitted'])){
		...
		if(count($errors_array) == 0){
			$sel_customer = "SELECT hat_customers_id, password, concat(first_name,' ', last_name) as full_name from hat_customers WHERE email = '$email'";
			...
			if(!$exec_sel_customer){
				...
				if(password_verify($password, $hashed_password)){
					//setcookie('hat_customers_id', $one_record['hat_customers_id'], time()+3600, '/', '', 0, 1);
					//setcookie('full_name', $one_record['full_name'], time()+3600, '/', '', 0, 1);
					$_SESSION['hat_customers_id'] = $one_record['hat_customers_id'];
